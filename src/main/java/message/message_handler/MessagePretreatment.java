...
public class MessagePretreatment {
    ...
    boolean messageCheck(Message message, List<Message> messageList) {
        IdentificationStage identificationStage=getMessageStage(message);
        Set<MessageType> set=getMessageSet(identificationStage);
        boolean a=messageList.containsAll(set);
        return a;
    }
